...
public class CreditsScreen extends ClickCrystalsBase {
    ...
    @Override
    protected void init() {
        ImageElement improperIssues = new ImageElement(TexturesIdentifiers.IMPROPERISSUES, nav.x + nav.width + 10, base.y + 10, 120, nav.height);
        ImageElement obvWolf = new ImageElement(TexturesIdentifiers.OBVWOLF, improperIssues.x + improperIssues.width + 90, nav.y + (int)(nav.height * 0.18), 75, (int)(nav.height * 0.80));
        this.addChild(improperIssues);
        this.addChild(obvWolf);

        Identifier descriptionBg = TexturesIdentifiers.SMOOTH_HORIZONTAL_WIDGET;
