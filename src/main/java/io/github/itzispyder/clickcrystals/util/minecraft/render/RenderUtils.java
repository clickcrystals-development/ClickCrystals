...
public final class RenderUtils implements Global {
    ...
    public static void fillRoundTabTop(GuiGraphicsExtractor context, int x, int y, int w, int h, int r, int color) {
        context.enableScissor(x, y, x + w, y + h);
        fillRoundRect(context, x, y, w, h + r, r, color);
        context.disableScissor();
    }

    public static void fillRoundTabBottom(GuiGraphicsExtractor context, int x, int y, int w, int h, int r, int color) {
        context.enableScissor(x, y, x + w, y + h);
        fillRoundRect(context, x, y - r, w, h + r, r, color);
        context.disableScissor();
    }

    public static void fillRoundHoriLine(GuiGraphicsExtractor context, int x, int y, int length, int thickness, int color) {
        fillRoundRect(context, x, y, length, thickness, thickness / 2, color);
    }

    public static void fillRoundVertLine(GuiGraphicsExtractor context, int x, int y, int length, int thickness, int color) {
    ...
    public static void drawHorLine(GuiGraphicsExtractor context, int x, int y, int length, int color) {
        fillRect(context, x, y, length, 1, color);
    }

    public static void drawVerLine(GuiGraphicsExtractor context, int x, int y, int length, int color) {
        fillRect(context, x, y, 1, length, color);
    }

    public static void drawRoundRect(GuiGraphicsExtractor context, int x, int y, int w, int h, int r, int color) {
        fillRoundShadow(context, x, y, w, h, r, 0.5F, color);
    }

    // default text

    public static void drawDefaultScaledText(GuiGraphicsExtractor context, Component text, int x, int y, float scale, boolean shadow, int color) {
    ...
    public static void drawDefaultCenteredScaledText(GuiGraphicsExtractor context, Component text, int centerX, int y, float scale, boolean shadow, int color) {
        Matrix3x2fStack m = context.pose().pushMatrix();
        m.scale(scale);
    ...
    public static void drawDefaultRightScaledText(GuiGraphicsExtractor context, Component text, int rightX, int y, float scale, boolean shadow, int color) {
        Matrix3x2fStack m = context.pose().pushMatrix();
        m.scale(scale);
    ...
    public static void drawDefaultScaledText(GuiGraphicsExtractor context, Component text, int x, int y, float scale, boolean shadow) {
        drawDefaultScaledText(context, text, x, y, scale, shadow, 0xFFFFFFFF);
    }

    public static void drawDefaultCenteredScaledText(GuiGraphicsExtractor context, Component text, int centerX, int y, float scale, boolean shadow) {
        drawDefaultCenteredScaledText(context, text, centerX, y, scale, shadow, 0xFFFFFFFF);
    }

    public static void drawDefaultRightScaledText(GuiGraphicsExtractor context, Component text, int rightX, int y, float scale, boolean shadow) {
        drawDefaultRightScaledText(context, text, rightX, y, scale, shadow, 0xFFFFFFFF);
    }

    public static void drawDefaultText(GuiGraphicsExtractor context, Component text, int x, int y, boolean shadow, int color) {
    ...
    public static void drawText(GuiGraphicsExtractor context, String text, int x, int y, float scale, boolean shadow) {
        drawDefaultScaledText(context, Component.literal(text), x, y, scale, shadow);
    }

    public static void drawText(GuiGraphicsExtractor context, String text, int x, int y, boolean shadow) {
        drawDefaultScaledText(context, Component.literal(text), x, y, 1.0F, shadow);
    }

    // draw right-aligned text

    public static void drawRightText(GuiGraphicsExtractor context, String text, int leftX, int y, float scale, boolean shadow) {
        drawDefaultRightScaledText(context, Component.literal(text), leftX, y, scale, shadow);
    }

    public static void drawRightText(GuiGraphicsExtractor context, String text, int leftX, int y, boolean shadow) {
        drawDefaultRightScaledText(context, Component.literal(text), leftX, y, 1.0F, shadow);
    }

    public static void drawRightText(GuiGraphicsExtractor context, Component text, int leftX, int y, float scale, boolean shadow) {
        drawDefaultRightScaledText(context, text, leftX, y, scale, shadow);
    }

    public static void drawRightText(GuiGraphicsExtractor context, Component text, int leftX, int y, boolean shadow) {
        drawDefaultRightScaledText(context, text, leftX, y, 1.0F, shadow);
    }

    // draw centered text

    public static void drawCenteredText(GuiGraphicsExtractor context, String text, int centerX, int y, float scale, boolean shadow) {
        drawDefaultCenteredScaledText(context, Component.literal(text), centerX, y, scale, shadow);
    }

    public static void drawCenteredText(GuiGraphicsExtractor context, String text, int centerX, int y, boolean shadow) {
        drawDefaultCenteredScaledText(context, Component.literal(text), centerX, y, 1.0F, shadow);
    }

    public static void drawCenteredText(GuiGraphicsExtractor context, Component text, int centerX, int y, float scale, boolean shadow) {
        drawDefaultCenteredScaledText(context, text, centerX, y, scale, shadow);
    }

    public static void drawCenteredText(GuiGraphicsExtractor context, Component text, int centerX, int y, boolean shadow) {
        drawDefaultCenteredScaledText(context, text, centerX, y, 1.0F, shadow);
    }

    // misc

    public static void drawTexture(GuiGraphicsExtractor context, Identifier texture, int x, int y, int w, int h) {
        context.blit(texture, x, y, 0, 0, w, h, w, h);
    }
    ...
    public static void drawItem(GuiGraphicsExtractor context, ItemStack item, int x, int y, float scale) {
        ...
        context.itemDecorations(mc.font, item, x, y);
        context.pose().popMatrix();
    }

    public static void drawItem(GuiGraphicsExtractor context, ItemStack item, int x, int y, float scale, String text) {
        x = (int)(x / scale);
        y = (int)(y / scale);
        context.pose().pushMatrix();
        context.pose().scale(scale, scale);
        context.item(item, x, y);
        context.itemDecorations(mc.font, item, x, y, text);
        context.pose().popMatrix();
    }

    public static void drawItem(GuiGraphicsExtractor context, ItemStack item, int x, int y, int size) {
